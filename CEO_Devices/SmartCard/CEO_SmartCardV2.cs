...
{
    internal class SmartCardV2 : ISmartCardProfile
	{
		...
		private SmartCardV2.FieldDescriptor[] tableField2 = new SmartCardV2.FieldDescriptor[]
		{
			new SmartCardV2.FieldDescriptor(4, 0, 13),
			new SmartCardV2.FieldDescriptor(17, 1, 100),
			new SmartCardV2.FieldDescriptor(17, 2, 100),
			new SmartCardV2.FieldDescriptor(17, 3, 100),
			new SmartCardV2.FieldDescriptor(17, 4, 100),
			new SmartCardV2.FieldDescriptor(117, 1, 100),
			new SmartCardV2.FieldDescriptor(117, 2, 100),
			new SmartCardV2.FieldDescriptor(117, 3, 100),
			new SmartCardV2.FieldDescriptor(117, 4, 100),
			new SmartCardV2.FieldDescriptor(217, 0, 8),
			new SmartCardV2.FieldDescriptor(225, 0, 1),
			new SmartCardV2.FieldDescriptor(5497, 1, 150),
			new SmartCardV2.FieldDescriptor(5497, 2, 150),
			new SmartCardV2.FieldDescriptor(5497, 3, 150),
			new SmartCardV2.FieldDescriptor(5497, 4, 150),
			new SmartCardV2.FieldDescriptor(5497, 5, 150),
			new SmartCardV2.FieldDescriptor(5497, 6, 150),
			new SmartCardV2.FieldDescriptor(5497, 7, 150),
			new SmartCardV2.FieldDescriptor(5497, 8, 150),
			new SmartCardV2.FieldDescriptor(246, 0, 100),
			new SmartCardV2.FieldDescriptor(359, 0, 8),
			new SmartCardV2.FieldDescriptor(367, 0, 8)
		};
		public SmartCardV2()
		{
		...
		public int LoadProfile(object control)
		{
			...
			if (progressBar != null)
			{
				...
				progressBar.Maximum = 5625;
				Application.DoEvents();
			}
			SystemException ex = new SystemException();
			try
			{
				int num = this.ReadBlock(1, 0, 226, 0);
				if (num < 0)
				{
					throw ex;
				}
				...
				num = this.ReadBlock(1, 246, 129, 246);
				if (num < 0)
				{
					throw ex;
				}
				...
				num = this.ReadBlock(0, 4, 150, 5497);
				if (num < 0)
				{
					throw ex;
				}
			...
			for (int i = 0; i < 150; i++)
			{
				if (this.cardBuffer[i + 5497] == 32)
				{
					this.cardBuffer[i + 5497] = 35;
					break;
				}
			}
			return 0;
		...
		public int LoadPhoto(object control)
		{
			...
			while (num4-- > 0)
			{
				num6 = this.ReadBlock(1, num2, 200, num3);
				if (num6 < 0)
				{
					break;
				}
			...
			if (num6 > 0 && num5 > 0)
			{
				num6 = this.ReadBlock(1, num2, (byte)num5, num3);
				num7 += num6;
				if (progressBar != null)
				{
					progressBar.Value = num7;
		...
		public string GetFieldValue(CEO_ProfileField fieldIndex)
		{
			...
			int num = this.tableField2[(int)fieldIndex].indexStart;
			int num2 = this.tableField2[(int)fieldIndex].indexStart + this.tableField2[(int)fieldIndex].size - 1;
			int i = num;
			string result = string.Empty;
			if (this.tableField2[(int)fieldIndex].subField > 0)
			{
				int num3 = 0;
				while (i < num2)
				{
					...
					{
						if (++num3 >= (int)this.tableField2[(int)fieldIndex].subField)
						{
							break;
						}
						num = i + 1;
		...
		public byte[] GetPhoto()
		{
			...
			Array.Copy(this.cardBuffer, 381, array, 0, 5118);
			return array;
		}
